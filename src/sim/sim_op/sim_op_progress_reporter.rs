...
impl ProgressReporterOp {
    ...
    fn calculate_avg_surface_temp(&self, sim: &Simulation) -> f64 {
        let temps: Vec<f64> = sim.cells.values()
            .map(|cell| cell.asth_layers_t[0].0.kelvin())
            .collect();
    ...
    fn calculate_total_energy(&self, sim: &Simulation) -> f64 {
        sim.cells.values()
            .map(|cell| {
                let asth_energy: f64 = cell.asth_layers_t.iter().map(|(current, _)| current.energy_joules()).sum();
                let lith_energy: f64 = cell.lith_layers_t.iter().map(|(current, _)| current.energy_joules()).sum();
