...
fn analyze_final_state(sim: &Simulation) {
    ...
    for cell in sim.cells.values() {
        ...
        let surface_temp = cell.layers[0].kelvin();
        ...
        // Total energy
        total_energy += cell.layers.iter().map(|l| l.energy_joules()).sum::<f64>();
        total_energy += cell.lithospheres.iter().map(|l| l.energy()).sum::<f64>();
    }
