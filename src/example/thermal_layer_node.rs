...
impl ThermalLayerNode {
    ...
    /// Uses the average melting point as the primary transition temperature
    pub fn get_material_phase(&self) -> MaterialPhase {
        let temp = self.kelvin();
        let composite = self.material_composite();

        if temp < composite.melting_point_avg_k {
            MaterialPhase::Solid
        } else if temp < composite.melting_point_avg_k + 1000.0 {
            MaterialPhase::Liquid
        } else {
            MaterialPhase::Gas
        }
    ...
    pub fn update_phase_from_kelvin(&mut self) {
        ...
        // Determine the appropriate phase based on temperature
        let new_phase = if temp < composite.melting_point_avg_k {
            MaterialPhase::Solid
        } else if temp < composite.melting_point_avg_k + 1000.0 {
            MaterialPhase::Liquid
        } else {
